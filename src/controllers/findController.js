import { findCustomerService, findProductService, findRoleService, findStateService, findStockService, findSupplierService, findTownship1Service, findTownshipService } from "../services/findService.js"
...
export const findTownship1Controller = async(req,res)=>{

    try {
        const allData = await findTownship1Service()
            return res.status(200).json({
                status:200,
                success:true,
                length:allData.length,
        ...
        console.log(error , 'find township controller error is')
           return res.status(500).json({
               status:500,
               success:false,
               message:error.message
